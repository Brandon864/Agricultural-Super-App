...
# TEMPORARY FIX FOR LOCAL DEVELOPMENT: Force SQLite.
# REMEMBER TO CHANGE THIS BACK BEFORE DEPLOYING TO RENDER!
app.config["SQLALCHEMY_DATABASE_URI"] = "sqlite:///agri_app.db"
# app.config["SQLALCHEMY_DATABASE_URI"] = os.environ.get("DATABASE_URL", "sqlite:///agri_app.db").replace("postgres://", "postgresql://")
...
# --- Diagnostic Print Statement (New) ---
# This will print the actual JWT_SECRET_KEY being used by Flask on startup.
# DO NOT leave this in production code, it's for debugging only.
print(f"DEBUG: Flask app is using JWT_SECRET_KEY: {app.config['JWT_SECRET_KEY']}")
...
# --- Flask-JWT-Extended Error Handlers (Added/Modified) ---
...
class User(db.Model):
    ...
    marketplace_items = db.relationship("MarketplaceItem", backref="seller", lazy=True, cascade="all, delete-orphan") # Corrected
        ...
        if file.filename != "" and allowed_file(file.filename):
            ...
            try:
                ...
                if new_profile_picture_url != user.profile_picture_url:
                    ...
                    updated = True
            except Exception as e: # This is the line that was causing the error
...
def leave_community(community_id):
    ...
    if not community:
        return jsonify({"message": "Community not found"}), 404
    membership = CommunityMembership.query.filter_by(user_id=int(user_id), community_id=community_id).first() # Corrected line: Removed the trailing colon
    if not membership:
